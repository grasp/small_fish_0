...
def append_history_data(date,symbol)

	source_file=File.expand_path("./history_daily_data3/#{date}/#{symbol}.txt",$raw_data)
	target_file=File.expand_path("./history_daily_data/#{symbol}.txt",$raw_data)
...
def append_all_history_data(date)
   count=0
	...
	$all_stock_list.keys.each do |symbol|
	  ...
	  raw_data_path=File.expand_path("./history_daily_data3/#{date}/#{symbol}.txt",$raw_data)
	  exsisted_data=File.expand_path("./history_daily_data/#{symbol}.txt",$raw_data)
	
	  if File.exists?(raw_data_path) && File.exists?(exsisted_data)
         append_history_data(date,symbol)
      end
    end
end
