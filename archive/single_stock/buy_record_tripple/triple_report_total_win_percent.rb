...
#只报告一次，避免重复报告
 def triple_report_total_win_percent(strategy,symbol,regenrate_flag)
   ...
   folder_report_triple=File.join(Strategy.send(strategy).root_path,symbol,Strategy.send(strategy).statistic,\
       Strategy.send(strategy).end_date,Strategy.send(strategy).win_expect,Strategy.send(strategy).count_freq,"report_triple")

   Dir.mkdir(folder_report_triple) unless File.exists?(folder_report_triple)

   report_file=File.join(Strategy.send(strategy).root_path,symbol,Strategy.send(strategy).statistic,\
       Strategy.send(strategy).end_date,Strategy.send(strategy).win_expect,Strategy.send(strategy).count_freq,"report_triple",expected_report)
   ...
   #加了一个是否需要重新生成报告的flag
    #  return if File.exists?(report_file) && regenrate_flag==false
      ...
      buy_record_folder=File.join(Strategy.send(strategy).root_path,symbol,Strategy.send(strategy).statistic,\
       Strategy.send(strategy).end_date,percent_num_day,count_freq,"buy_record_triple")
      Dir.new(buy_record_folder) unless File.exists?(buy_record_folder)
