...
require File.expand_path("../../statistics/double_history_statistic.rb",__FILE__)
...
def generate_year_statistic(strategy,symbol,year)
    ...
    win_expect=Strategy.send(strategy).win_expect
    count_freq=Strategy.send(strategy).count_freq
    win_expect_folder=File.join(Strategy.send(strategy).root_path,symbol,Strategy.send(strategy).statistic,\
    Strategy.send(strategy).end_date,win_expect)
     Dir.mkdir(win_expect_folder) unless File.exists?(win_expect_folder)
